...
const HeaderMain = () => {
  ...
  const countLike = photos.filter((item) => item.isLike)?.length;
  const countCart = cart?.length;

  const handleSignOut = () => setUsers(null);
  return (
    <div className="p-4 shadow-md flex items-center justify-center transition-all">
      {user ? (
        <div className="flex items-center justify-center gap-3">
          <img
            src="https://images.unsplash.com/photo-1664575600850-c4b712e6e2bf?ixlib=rb-4.0.3&ixid=MnwxMjA3fDF8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=774&q=80"
      ...
      <div className="ml-auto mr-5 flex gap-3 items-center">
        <span className="relative">
          ...
          <span className="w-4 h-4 rounded-full text-white bg-pink-400 absolute flex justify-center items-center text-xs top-0 right-0 translate-x-2/4 -translate-y-2/4">
            {countCart}
          </span>
        </span>
        <span className="relative">
